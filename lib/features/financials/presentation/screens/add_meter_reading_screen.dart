...
class AddMeterReadingScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<AddMeterReadingScreen> createState() => _AddMeterReadingScreenState();
}
...
class _AddMeterReadingScreenState extends ConsumerState<AddMeterReadingScreen> {
  ...
  void _submit() {
    if (_formKey.currentState!.validate() && _date != null) {
      ref.read(meterReadingsControllerProvider.notifier).addReading(
        buildingId: _selectedBuildingId,
        apartmentId: _selectedApartmentId,
        date: _date!,
        previousReading: double.tryParse(_previousReadingController.text.trim()) ?? 0,
        currentReading: double.tryParse(_currentReadingController.text.trim()) ?? 0,
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<AsyncValue<void>>(
      meterReadingsControllerProvider,
      (_, state) {
        state.whenOrNull(
          data: (_) => context.pop(),
              ...
              apartmentsAsync.when(
                data: (apartments) {
                  ...
                  return DropdownButtonFormField<String>(
                    ...
                    validator: (v) => v == null && !_isSharedExpense ? 'مطلوب' : null,
                  );
                },
